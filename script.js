...
function openStory(id){const story=cache.stories.find(x=>x.id===id);if(!story)return;const m=$('storyViewer');if(!m)return;const u=story.profile||{};const replyDisabled=story.user_id===user.id; $('storyViewerContent').innerHTML=`<div class="storyViewerHead"><span class="profileIdentityLink" onclick="viewPublicProfileAny('${story.user_id}')"><div class="avatar storyViewerAvatar">${u.avatar_url?`<img src="${esc(u.avatar_url)}">`:initials(u.name||'Pengguna')}</div></span><div><b class="profileLink" onclick="viewPublicProfileAny('${story.user_id}')">${esc(u.name||'Pengguna')}</b><div class="muted">${new Date(story.created_at).toLocaleString('id-ID')}</div></div></div>${story.image_url?`<img class="storyViewerMedia" src="${esc(story.image_url)}" alt="Story">`:''}${story.video_url?`<video class="storyViewerMedia" controls autoplay src="${esc(story.video_url)}"></video>`:''}${story.body?`<div class="storyViewerText">${esc(story.body)}</div>`:''}<div class="storyReplyBox">${replyDisabled?'<span class="muted">Ini Story Anda</span>':`<input id="storyReplyText" class="field" placeholder="Balas Story ini..." onkeydown="if(event.key==='Enter')replyToStory('${story.id}')"><button class="primary" onclick="replyToStory('${story.id}')">Balas</button>`}</div>`;m.classList.add('show')}
async function replyToStory(id){const story=cache.stories.find(x=>x.id===id);const input=$('storyReplyText');const body=input?.value.trim();if(!story||!body)return;if(story.user_id===user.id)return;const {data:found,error}=await sb.rpc('start_direct_conversation',{target_user:story.user_id});if(error)return alert('Percakapan belum dapat dibuat: '+error.message);currentConversation=found;const message=`Balasan Story: ${body}`;const {error:sendError}=await sb.from('messages').insert({conversation_id:found,sender_id:user.id,body:message});if(sendError)return alert('Balasan belum terkirim: '+sendError.message);closeStoryViewer();currentPage='chat';document.querySelectorAll('.navbtn').forEach(x=>x.classList.toggle('active',x.dataset.page==='chat'));await renderChat()}
...
async function loadComments(id){
 ...
 const {data:pub}=ids.length?await sb.from('public_profiles').select('id,name,avatar_url').in('id',ids):{data:[]};
 const map=Object.fromEntries((pub||[]).map(x=>[x.id,x]));
 box.innerHTML=(data||[]).map(c=>`<div class="commentLine"><span class="profileIdentityLink" onclick="viewPublicProfileAny('${c.user_id}')"><div class="avatar" style="width:32px;height:32px;flex-basis:32px">${map[c.user_id]?.avatar_url?`<img class="avatar" src="${esc(map[c.user_id].avatar_url)}">`:initials(map[c.user_id]?.name)}</div></span><div class="commentBubble"><div class="commentName profileLink" onclick="viewPublicProfileAny('${c.user_id}')">${esc(map[c.user_id]?.name||'Pengguna')}</div><div>${esc(c.body)}</div></div></div>`).join('')+
...
async function viewPublicProfileAny(id){
  if(!id)return;
  if(id===user?.id){currentPage='profile';await showPage('profile');return}
  const {data:f,error}=await sb.from('public_profiles').select('id,name,username,avatar_url,city,purpose,bio,guardian_score').eq('id',id).maybeSingle();
  if(error||!f){return alert('Profil pengguna tidak dapat dibuka.')}
  await loadFollowStats([id,user.id]);
  $('main').innerHTML=`<div class="card publicProfileCard">
    <button class="ghost" onclick="currentPage='home';showPage('home')">← Kembali</button>
    <div class="publicProfileHero">
      <div class="publicProfileAvatar">${f.avatar_url?'<img src="'+esc(f.avatar_url)+'">':initials(f.name)}</div>
      <h1 class="profileLink" onclick="viewPublicProfileAny('${f.id}')">${esc(f.name||'Pengguna Guardian Angel')}</h1>
      ${f.username?'<div class="muted">@'+esc(f.username)+'</div>':''}
      ${f.city?'<div class="muted">⌖ '+esc(f.city)+'</div>':''}
      ${followStatsHTML(f.id)}
      <p>${esc(f.bio||f.purpose||'Saling menguatkan, mendoakan, dan bertumbuh bersama.')}</p>
      <div class="row publicProfileActions" style="justify-content:center">${followButtonHTML(f.id)}<button class="primary" onclick="startChat('${f.id}')">Balas / Pesan</button><button class="ghost" onclick="blockUser('${f.id}')">Blokir</button><button class="ghost" onclick="reportUser('${f.id}')">Laporkan</button></div>
    </div>
  </div>`;
}
...
async function renderProfile(){
 ...
 '<div class="profileCoverNew" style="'+(p.cover_url?'background-image:url("'+esc(p.cover_url)+'")':'')+'"><div class="profileCoverShade"></div><label class="profileCoverEdit" title="Ubah foto sampul">▧ Ubah sampul<input id="coverFileTop" type="file" accept="image/jpeg,image/png,image/webp" hidden></label></div>'+
 ...
 '<div class="profileMainInfo"><h1 class="profileLink" onclick="viewPublicProfileAny(\''+user.id+'\')">'+esc(p.name||'Pengguna Guardian Angel')+'</h1><p class="profileBio">'+esc(p.bio||'Saling menguatkan, mendoakan, dan bertumbuh bersama.')+'</p>'+
...
async function saveProfile(){
 ...
 const file=$('avatarFile')?.files?.[0] || $('avatarFileTop')?.files?.[0]; const coverFile=$('coverFileTop')?.files?.[0];
 if(file){
 ...
 if(coverFile){
   if(coverFile.size>8*1024*1024)return alert('Ukuran foto sampul maksimal 8 MB.');
   const ext=(coverFile.name.split('.').pop()||'jpg').toLowerCase();
   const path=user.id+'/cover-'+crypto.randomUUID()+'.'+ext;
   const {error:up}=await sb.storage.from('avatars').upload(path,coverFile,{upsert:false,contentType:coverFile.type});
   if(up)return alert('Foto sampul gagal disimpan: '+up.message);
   const {data:pub}=sb.storage.from('avatars').getPublicUrl(path); patch.cover_url=pub.publicUrl;
 }
